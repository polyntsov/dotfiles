function fish_prompt --description 'Write out the prompt'
    ...
    if set -l git_branch (command git symbolic-ref HEAD 2>/dev/null | string replace refs/heads/ '')
        ...
        if command git diff-index --quiet HEAD --
            ...
            set git_status (set_color white):
        end
    ...
    #set_color -b black
    printf '%s%s%s%s%s%s%s%s%s%s%s%s%s' (set_color -o white) (set_color green) $USER (set_color white) ':' (set_color blue) (prompt_pwd) (set_color white) $git_info (set_color white) (set_color white)
